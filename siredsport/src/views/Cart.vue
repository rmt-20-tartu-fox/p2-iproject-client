<template>
  <div>
    <Navbar />
    <div>
      <h1 style="font-weight: bold; color: white; margin-top: -50px;">Your Cart</h1>
      ...
      <div class="text-right fixed-bottom mx-5 my-5">
        <b-button-group class="text-right" v-if="carts.length">
          ...
          <b-button variant="warning" class="mx-1" @click="checkout">Checkout here!</b-button>
    ...
    <div class="row d-flex justify-content-start mt-5 mx-2">
      <div class="col-6" v-for="cart in carts" :key="cart.id">
        <b-card no-body class="overflow-hidden mt-2" style="max-height: 320px; max-width: 640px;">
...
export default {
  ...
  methods: {
    ...
    checkout() {
      this.$store.dispatch("checkoutPreview")
      this.$router.push({
        name: "Checkout"
      })
    }
